...
static uint8_t L9963E_utils_wait_for_conversion(void)
{
    ...
    while ((GetTickMs() - startTime) <
           L9963E_UTILS_MEASUREMENT_TIMEOUT_MS)
    {
 ...
 */
static uint8_t L9963E_utils_read_cell_with_timeout(
...
/*
 * Read one GPIO ADC channel and wait until fresh data
 * is available.
 */
static uint8_t L9963E_utils_read_gpio_with_timeout(
 ...
 */
static uint8_t L9963E_utils_read_battery_with_timeout(
...
uint8_t L9963E_utils_init(void)
{
    ...
    /*
     * The same register layout is used for the
     * individual-cell and total-pack voltage threshold
     * configuration.
     */
    L9963E_RegisterUnionTypeDef THRESH;

    THRESH.generic =
        L9963E_VCELL_THRESH_UV_OV_DEFAULT;

    /*
     * Wide low-level thresholds retained from the
     * existing hardware bring-up configuration.
     *
     * Application-level UV/OV protection is performed
     * separately in bms_app.c.
     */
    THRESH.VCELL_THRESH_UV_OV.threshVcellOV =
        0b11111111;

    THRESH.VCELL_THRESH_UV_OV.threshVcellUV =
        0;
    ...
    /*
     * Perform L9963E addressing.
     */
    status = L9963E_addressing_procedure(
        &h9l,
        0b11,
        0,
        0,
    ...
    /*
     * Longer communication timeout retained for
     * hardware bring-up.
     */
    status = L9963E_setCommTimeout(
        &h9l,
        _2048MS,
        L9963E_DEVICE_BROADCAST,
        0);
    ...
    /*
     * Configure the currently selected physical
     * measurement channels.
     */
    status = L9963E_set_enabled_cells(
        &h9l,
        0x1,
        ENABLED_CELLS);
    ...
    /*
     * Configure GPIO3 through GPIO9.
     */
    status = L9963E_DRV_reg_write(
        &(h9l.drv_handle),
        0x1,
        L9963E_GPIO9_3_CONF_ADDR,
        &GPIOCONFIG,
        10,
        0);
    ...
    /*
     * Configure individual cell-voltage thresholds.
     */
    status = L9963E_DRV_reg_write(
        &(h9l.drv_handle),
        0x1,
        L9963E_VCELL_THRESH_UV_OV_ADDR,
        &THRESH,
        10,
        0);
    ...
    /*
     * Configure total battery-voltage thresholds.
     */
    status = L9963E_DRV_reg_write(
        &(h9l.drv_handle),
        0x1,
        L9963E_VBATT_SUM_TH_ADDR,
        &THRESH,
        10,
        0);
    ...
    /*
     * VREF will eventually be required for the NTC path.
     * Leave disabled until the temperature hardware
     * configuration is confirmed.
     */
    // status = L9963E_enable_vref(
    //     &h9l,
    //     L9963E_DEVICE_BROADCAST,
    //     0);
...
uint8_t L9963E_utils_read_cells(
    uint8_t read_gpio)
{
    /*
     ...
     */
    uint16_t newCells[CELLS_N] = {0};

    uint16_t newGpios[GPIOS_N] = {0};
    ...
    /*
     * Wait for conversion completion without allowing
     * the MCU to hang forever.
     */
    if (!L9963E_utils_wait_for_conversion())
    {
        return 0U;
    }
    ...
    /*
     * Acquire the seven cell channels used by the
     * project's 7S application mapping.
     */

    if (!L9963E_utils_read_cell_with_timeout(
            L9963E_CELL1,
            &newCells[0]))
    {
        return 0U;
     ...
     */
    if (!L9963E_utils_read_battery_with_timeout(
            &newVtot,
            &newVsumbatt))
    {
        return 0U;
    ...
    /*
     * If GPIO/temperature measurements were requested,
     * acquire GPIO3 through GPIO9.
     */
    if (read_gpio)
    {
        if (!L9963E_utils_read_gpio_with_timeout(
                L9963E_GPIO3,
                &newGpios[0]))
        {
            return 0U;
...
uint16_t const *L9963E_utils_get_gpios(
    uint8_t *len)
{
    if (len)
    {
...
uint16_t const *L9963E_utils_get_cells(
    uint8_t *len)
{
    if (len)
    {
...
float L9963E_utils_get_cell_mv(
    uint8_t index)
{
    ...
    L9963E_CoulombData_t *data)
{
    ...
    data->sampleCount =
        (uint16_t)
        burstData._0x7B.Frame1.CoulombCntTime;


    data->overflow =
        (uint8_t)
        burstData._0x7B.Frame1.CoCouOvF;
    ...
    uint32_t accumulatorRaw =
        ((uint32_t)
         burstData._0x7B.Frame2.CoulombCounter_msb << 16) |
        ((uint32_t)
         burstData._0x7B.Frame3.CoulombCounter_lsb);
    ...
    uint32_t synchRaw =
        (uint32_t)
        burstData._0x7B.Frame4.CUR_INST_synch;
    ...
    uint32_t calibRaw =
        (uint32_t)
        burstData._0x7B.Frame5.CUR_INST_calib;
